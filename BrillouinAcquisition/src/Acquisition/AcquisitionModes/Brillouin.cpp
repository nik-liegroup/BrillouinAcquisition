...
void Brillouin::acquire(std::unique_ptr <StorageWrapper>& storage) {
	...
	if (m_scanControl) {
		// Use the same anchor as the live preview, including the scanner-scale
		// correction accumulated by objective switches before Start was pressed.
		auto stagePosition = m_scanControl->getPosition(PositionType::STAGE);
		auto anchor = m_scanControl->getRelativeGridAnchorUm();
		m_startPosition = POINT3{ stagePosition.x + anchor.x, stagePosition.y + anchor.y, stagePosition.z };
