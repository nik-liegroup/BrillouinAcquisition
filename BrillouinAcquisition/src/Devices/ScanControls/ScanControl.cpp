...
POINT2 ScanControl::getPositionOffset(bool positionIsAbsolute) {
	...
	//
	// getActiveObjectiveFovOffsetUm() is added ONLY in the live-preview relative branch below,
	// nowhere else - it means two different things depending on what's being converted:
	//
	...
	//
	// - Absolute targets and m_startPosition (both measurement-mode branches, and the idle
	// absolute branch) are real stage-frame physical locations with no such convention, and no
	// longer have any live fovOffsetUm folded in anywhere (see Brillouin::resolvedGridOriginUm()
	// and Brillouin::acquire()'s own comments for the full reasoning, operator-confirmed): a
